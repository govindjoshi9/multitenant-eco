'use client'
import Link from "next/link"
import { Poppins } from "next/font/google"
import { useState } from "react"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { usePathname } from "next/navigation"
import { NavbarSidbar } from "./nav-sidbar"
import { MenuIcon } from "lucide-react"

const poppines = Poppins({
  subsets: ['latin'],
  weight: ["700"],
})


interface NavbarItemProps {
  href: string;
  children: React.ReactNode;
  isActive?: boolean;
}
const NavbarItem = ({
  href,
  children,
  isActive
}: NavbarItemProps) => {
  return (
    <Button variant="outline"
      className={cn("bg-transparent hover:bg-transparent rounded-full hover:border-primary border-transparent px-3.5 text-lg",
        isActive && "bg-black text-white hover:bgb hover:text-white"
      )}>
      <Link href={href}>
        {children}
      </Link>
    </Button>
  )
}

const navbarItems = [
  { href: "/", children: "Home" },
  { href: "/about", children: "About" },
  { href: "/features", children: "Features" },
  { href: "/pricing", children: "Princing" },
  { href: "/contact", children: "Contact" },
]


const Navbar = () => {
  const pasthname = usePathname();
  const [isSidbarOpen, setSidbarOpen ] =  useState(false)
  return (
    <nav>

      <div className="h-20 flex border-b justify-between font-medium bg-white">

        <Link href='/' className="pl-6 flex items-center">
          <span className={cn("text-5xl font-semibold", poppines.className)}>
            funroad
          </span>
        </Link>

        <NavbarSidbar
        item={navbarItems}
       open={isSidbarOpen}
       OnOpenChange={setSidbarOpen} 
        />

        <div className="items-center gap-4 hidden lg:flex">
          {navbarItems.map((item) => (
            <NavbarItem
              key={item.href}
              {...item}
              isActive={pasthname === item.href}
            >
              {item.children}
            </NavbarItem>
          ))}
        </div>
        <div className="hidden lg:flex">
          <Button variant={"secondary"}
            className="border-l border-t-0 border-r-0 px-12 h-full rounded-none bg-white
           hover:bg-pink-400 transition-colors text-lg "
          >
            <Link href={"/sign-in"}>
            Login
            </Link>
          </Button>
          <Button
            className="border-l border-t-0 border-r-0 px-12 h-full rounded-none bg-black
          text-white hover:bg-pink-400 hover:text-black transition-colors text-lg ">
            <Link href={"/sign-in"}>
            Start selling
            </Link>
          </Button>
        </div>
        <div className="flex lg:hidden items-center justify-center">
          <Button
          variant={"ghost"}
          className="size-12 border-transparent bg-white"
          onClick={()=> setSidbarOpen(true)}
          >
            
            <MenuIcon/>
          </Button>
        </div>
      </div>
    </nav>
  )
}

export default Navbar
